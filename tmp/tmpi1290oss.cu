...
extern "C" __global__ void __launch_bounds__(128, 1) kernel_kernel(bfloat16_t* __restrict__ K, bfloat16_t* __restrict__ O, bfloat16_t* __restrict__ Q, bfloat16_t* __restrict__ V, int* __restrict__ cu_seqlens_k, int* __restrict__ cu_seqlens_q, int max_seqlen_q) {
  ...
  for (int kv_block_idx = 0; kv_block_idx < min(((((int)blockIdx.x) * 4) + (((cu_seqlens_q[1] + 319) - cu_seqlens_q[0]) >> 6)), (((cu_seqlens_k[1] + 63) - cu_seqlens_k[0]) >> 6)); ++kv_block_idx) {
    ...
    for (int i_5 = 0; i_5 < 16; ++i_5) {
      ...
      *(uint2*)(((bfloat16_t*)buf_dyn_shmem) + ((((((((((((int)threadIdx.x) & 31) >> 4) * 4096) + (i_5 * 256)) + ((((int)threadIdx.x) >> 5) * 64)) + (((((((int)threadIdx.x) & 15) >> 3) + (i_5 & 1)) & 1) * 32)) + ((((((int)threadIdx.x) >> 6) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 16)) + (((((((int)threadIdx.x) & 63) >> 5) + ((((int)threadIdx.x) & 3) >> 1)) & 1) * 8)) + ((((int)threadIdx.x) & 1) * 4)) + 32768)) = condval_1;
    }
    #pragma unroll
    for (int i_6 = 0; i_6 < 128; ++i_6) {
      float condval_2;
      if ((((((((((int)blockIdx.x) * 256) + (((i_6 & 15) >> 2) * 64)) + ((((int)threadIdx.x) >> 6) * 32)) + 32) <= ((((kv_block_idx * 64) + ((i_6 >> 4) * 8)) + ((((int)threadIdx.x) & 3) * 2)) + (i_6 & 1))) || ((cu_seqlens_q[1] - cu_seqlens_q[0]) <= (((((((int)blockIdx.x) * 256) + (((i_6 & 15) >> 2) * 64)) + ((((int)threadIdx.x) >> 5) * 16)) + (((i_6 & 3) >> 1) * 8)) + ((((int)threadIdx.x) & 31) >> 2)))) || ((cu_seqlens_k[1] - cu_seqlens_k[0]) <= ((((kv_block_idx * 64) + ((i_6 >> 4) * 8)) + ((((int)threadIdx.x) & 3) * 2)) + (i_6 & 1))))) {
    ...
    for (int i_9 = 0; i_9 < 8; ++i_9) {
      ...
      for (int rv = 0; rv < 16; ++rv) {
        scores_max[i_9] = max(scores_max[i_9], acc_score[((((rv & 7) * 16) + (i_9 * 2)) + (rv >> 3))]);
      }
    ...
    for (int i_13 = 0; i_13 < 8; ++i_13) {
      ...
      for (int rv_1 = 0; rv_1 < 16; ++rv_1) {
        scores_sum[i_13] = (scores_sum[i_13] + acc_score[((((rv_1 & 7) * 16) + (i_13 * 2)) + (rv_1 >> 3))]);
      }
    ...
    for (int i_17 = 0; i_17 < 16; ++i_17) {
      ...
      *(uint2*)(((bfloat16_t*)buf_dyn_shmem) + ((((((((((((int)threadIdx.x) & 31) >> 4) * 4096) + (i_17 * 256)) + ((((int)threadIdx.x) >> 5) * 64)) + (((((((int)threadIdx.x) & 15) >> 3) + (i_17 & 1)) & 1) * 32)) + ((((((int)threadIdx.x) >> 6) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 16)) + (((((((int)threadIdx.x) & 63) >> 5) + ((((int)threadIdx.x) & 3) >> 1)) & 1) * 8)) + ((((int)threadIdx.x) & 1) * 4)) + 40960)) = condval_3;
    }
    __syncthreads();
