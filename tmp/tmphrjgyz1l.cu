...
extern "C" __global__ void __launch_bounds__(128, 1) kernel_kernel(bfloat16_t* __restrict__ K, bfloat16_t* __restrict__ O, bfloat16_t* __restrict__ Q, bfloat16_t* __restrict__ V, int* __restrict__ cu_seqlens_k, int* __restrict__ cu_seqlens_q, int max_seqlen_q) {
  ...
  for (int kv_block_idx = 0; kv_block_idx < (min(((((((int)blockIdx.x) * 64) + cu_seqlens_q[1]) + 319) - cu_seqlens_q[0]), ((cu_seqlens_k[1] + 255) - cu_seqlens_k[0])) >> 8); ++kv_block_idx) {
    __syncthreads();
    #pragma unroll
    ...
    for (int i_6 = 0; i_6 < 128; ++i_6) {
      float condval_2;
      if (((((((((int)blockIdx.x) * 64) + ((((int)threadIdx.x) >> 6) * 32)) + 32) <= ((((kv_block_idx * 256) + ((i_6 >> 2) * 8)) + ((((int)threadIdx.x) & 3) * 2)) + (i_6 & 1))) || ((cu_seqlens_q[1] - cu_seqlens_q[0]) <= ((((((int)blockIdx.x) * 64) + ((((int)threadIdx.x) >> 5) * 16)) + (((i_6 & 3) >> 1) * 8)) + ((((int)threadIdx.x) & 31) >> 2)))) || ((cu_seqlens_k[1] - cu_seqlens_k[0]) <= ((((kv_block_idx * 256) + ((i_6 >> 2) * 8)) + ((((int)threadIdx.x) & 3) * 2)) + (i_6 & 1))))) {
...
#endif
    *(uint1*)(((bfloat16_t*)buf_dyn_shmem) + ((((((((((i_19 >> 4) * 4096) + ((((int)threadIdx.x) >> 5) * 1024)) + ((i_19 & 1) * 512)) + (((((int)threadIdx.x) & 31) >> 2) * 64)) + (((((((int)threadIdx.x) & 31) >> 4) + ((i_19 & 15) >> 3)) & 1) * 32)) + (((((((int)threadIdx.x) & 15) >> 3) + ((i_19 & 7) >> 2)) & 1) * 16)) + (((((((int)threadIdx.x) & 7) >> 2) + ((i_19 & 3) >> 1)) & 1) * 8)) + ((((int)threadIdx.x) & 3) * 2)) + 65536)) = __2;
  }
  __syncthreads();
  #pragma unroll
