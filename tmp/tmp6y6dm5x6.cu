...
extern "C" __global__ void __launch_bounds__(128, 1) kernel_kernel(bfloat16_t* __restrict__ K, bfloat16_t* __restrict__ O, bfloat16_t* __restrict__ Q, bfloat16_t* __restrict__ V, int* __restrict__ cu_seqlens_k, int* __restrict__ cu_seqlens_q, int max_seqlen_q) {
  ...
  for (int kv_block_idx = 0; kv_block_idx < (min(((((((int)blockIdx.x) * 64) + cu_seqlens_q[1]) + 191) - cu_seqlens_q[0]), ((cu_seqlens_k[1] + 127) - cu_seqlens_k[0])) >> 7); ++kv_block_idx) {
    __syncthreads();
    #pragma unroll
    for (int i_5 = 0; i_5 < 32; ++i_5) {
      ...
      *(uint2*)(((bfloat16_t*)buf_dyn_shmem) + (((((((((((int)threadIdx.x) & 31) >> 4) * 8192) + (i_5 * 256)) + ((((int)threadIdx.x) >> 5) * 64)) + (((((((int)threadIdx.x) & 15) >> 3) + (i_5 & 1)) & 1) * 32)) + ((((((int)threadIdx.x) >> 6) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 16)) + (((((((int)threadIdx.x) & 63) >> 5) + ((((int)threadIdx.x) & 3) >> 1)) & 1) * 8)) + ((((int)threadIdx.x) & 1) * 4))) = condval_1;
    }
    #pragma unroll
    for (int i_6 = 0; i_6 < 64; ++i_6) {
      float condval_2;
      if (((((((((int)blockIdx.x) * 64) + ((((int)threadIdx.x) >> 6) * 32)) + 32) <= ((((kv_block_idx * 128) + ((i_6 >> 2) * 8)) + ((((int)threadIdx.x) & 3) * 2)) + (i_6 & 1))) || ((cu_seqlens_q[1] - cu_seqlens_q[0]) <= ((((((int)blockIdx.x) * 64) + ((((int)threadIdx.x) >> 5) * 16)) + (((i_6 & 3) >> 1) * 8)) + ((((int)threadIdx.x) & 31) >> 2)))) || ((cu_seqlens_k[1] - cu_seqlens_k[0]) <= ((((kv_block_idx * 128) + ((i_6 >> 2) * 8)) + ((((int)threadIdx.x) & 3) * 2)) + (i_6 & 1))))) {
    ...
    for (int i_17 = 0; i_17 < 32; ++i_17) {
      ...
      *(uint2*)(((bfloat16_t*)buf_dyn_shmem) + ((((((((((((int)threadIdx.x) & 31) >> 4) * 8192) + (i_17 * 256)) + ((((int)threadIdx.x) >> 5) * 64)) + (((((((int)threadIdx.x) & 15) >> 3) + (i_17 & 1)) & 1) * 32)) + ((((((int)threadIdx.x) >> 6) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 16)) + (((((((int)threadIdx.x) & 63) >> 5) + ((((int)threadIdx.x) & 3) >> 1)) & 1) * 8)) + ((((int)threadIdx.x) & 1) * 4)) + 16384)) = condval_3;
    }
    __syncthreads();
...
#endif
    *(uint1*)(((bfloat16_t*)buf_dyn_shmem) + ((((((((((i_19 >> 4) * 4096) + ((((int)threadIdx.x) >> 5) * 1024)) + ((i_19 & 1) * 512)) + (((((int)threadIdx.x) & 31) >> 2) * 64)) + (((((((int)threadIdx.x) & 31) >> 4) + ((i_19 & 15) >> 3)) & 1) * 32)) + (((((((int)threadIdx.x) & 15) >> 3) + ((i_19 & 7) >> 2)) & 1) * 16)) + (((((((int)threadIdx.x) & 7) >> 2) + ((i_19 & 3) >> 1)) & 1) * 8)) + ((((int)threadIdx.x) & 3) * 2)) + 32768)) = __2;
  }
  __syncthreads();
  #pragma unroll
  for (int i_20 = 0; i_20 < 64; ++i_20) {
    if (((((int)blockIdx.x) * 64) + i_20) < (cu_seqlens_q[1] - cu_seqlens_q[0])) {
      if (0 <= (((((int)blockIdx.x) * 64) + i_20) + cu_seqlens_q[0])) {
        if ((((i_20 + cu_seqlens_q[0]) >> 6) + ((int)blockIdx.x)) < 1) {
