...
extern "C" __global__ void __launch_bounds__(256, 1) kernel_kernel(bfloat16_t* __restrict__ K, bfloat16_t* __restrict__ O, bfloat16_t* __restrict__ Q, bfloat16_t* __restrict__ V, int* __restrict__ cu_seqlens_k, int* __restrict__ cu_seqlens_q, int max_seqlen_q) {
  ...
  for (int kv_block_idx = 0; kv_block_idx < min(((((int)blockIdx.x) * 2) + (((cu_seqlens_q[1] + 383) - cu_seqlens_q[0]) >> 7)), (((cu_seqlens_k[1] + 127) - cu_seqlens_k[0]) >> 7)); ++kv_block_idx) {
    ...
    for (int i_6 = 0; i_6 < 128; ++i_6) {
      float condval_2;
      if ((((((((((int)blockIdx.x) * 256) + (((i_6 & 7) >> 2) * 128)) + ((((int)threadIdx.x) >> 6) * 32)) + 32) <= ((((kv_block_idx * 128) + ((i_6 >> 3) * 8)) + ((((int)threadIdx.x) & 3) * 2)) + (i_6 & 1))) || ((cu_seqlens_q[1] - cu_seqlens_q[0]) <= (((((((int)blockIdx.x) * 256) + (((i_6 & 7) >> 2) * 128)) + ((((int)threadIdx.x) >> 5) * 16)) + (((i_6 & 3) >> 1) * 8)) + ((((int)threadIdx.x) & 31) >> 2)))) || ((cu_seqlens_k[1] - cu_seqlens_k[0]) <= ((((kv_block_idx * 128) + ((i_6 >> 3) * 8)) + ((((int)threadIdx.x) & 3) * 2)) + (i_6 & 1))))) {
    ...
    for (int i_17 = 0; i_17 < 16; ++i_17) {
      ...
      *(uint2*)(((bfloat16_t*)buf_dyn_shmem) + ((((((((((((int)threadIdx.x) & 31) >> 4) * 8192) + (i_17 * 512)) + ((((int)threadIdx.x) >> 5) * 64)) + ((((((int)threadIdx.x) >> 7) + ((((int)threadIdx.x) & 15) >> 3)) & 1) * 32)) + (((((((int)threadIdx.x) & 127) >> 6) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 16)) + (((((((int)threadIdx.x) & 63) >> 5) + ((((int)threadIdx.x) & 3) >> 1)) & 1) * 8)) + ((((int)threadIdx.x) & 1) * 4)) + 49152)) = condval_3;
    }
    __syncthreads();
