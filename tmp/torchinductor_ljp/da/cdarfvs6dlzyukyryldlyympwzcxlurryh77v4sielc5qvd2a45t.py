...
# kernel path: /home/ljp/llm/Diffulex/tmp/torchinductor_ljp/5n/c5nuhmuoirxqwdt7icoc45m2mzoeuwkqvsc457s7nwdz5j37z4hh.py
# Topologically Sorted Source Nodes: [to, to_1, x, pow_1, var, add, rsqrt, mul_, to_3, x_1, residual], Original ATen: [aten._to_copy, aten.add, aten.pow, aten.mean, aten.rsqrt, aten.mul]
# Source node to ATen node mapping:
#   add => add_1
#   mul_ => mul
#   pow_1 => pow_1
#   residual => convert_element_type_2
#   rsqrt => rsqrt
...
#   %convert_element_type_1 : [num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%arg1_1, torch.float32), kwargs = {})
#   %add : [num_users=3] = call_function[target=torch.ops.aten.add.Tensor](args = (%convert_element_type, %convert_element_type_1), kwargs = {})
...
#   %mul_1 : [num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%convert_element_type_3, %arg2_1), kwargs = {})
...
@triton.jit
def triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_0(in_ptr0, in_ptr1, in_ptr2, out_ptr1, out_ptr2, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    xnumel = 64
    rnumel = 3584
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        tmp2 = tl.load(in_ptr1 + (r1 + 3584*x0), rmask & xmask, eviction_policy='evict_last', other=0.0).to(tl.float32)
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r1 = rindex
        tmp9 = tl.load(in_ptr0 + (r1 + 3584*x0), rmask & xmask, eviction_policy='evict_first', other=0.0).to(tl.float32)
        tmp11 = tl.load(in_ptr1 + (r1 + 3584*x0), rmask & xmask, eviction_policy='evict_first', other=0.0).to(tl.float32)
        tmp21 = tl.load(in_ptr2 + (r1), rmask, eviction_policy='evict_last', other=0.0).to(tl.float32)
        ...
        tmp22 = tmp20 * tmp21
        tmp23 = tmp13.to(tl.float32)
        tl.store(out_ptr1 + (r1 + 3584*x0), tmp22, rmask & xmask)
        tl.store(out_ptr2 + (r1 + 3584*x0), tmp23, rmask & xmask)
''', device_str='cuda')
...
def call(args):
    ...
    assert_size_stride(arg1_1, (64, 3584), (3584, 1))
    ...
    with torch.cuda._DeviceGuard(0):
        ...
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_0.run(arg0_1, arg1_1, arg2_1, buf1, buf2, 64, 3584, grid=grid(64), stream=stream0)
        del arg0_1
        del arg1_1
        del arg2_1
    return (buf1, buf2, )
